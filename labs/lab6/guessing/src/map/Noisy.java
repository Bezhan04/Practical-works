...
public class Noisy {
    public int seed; public float octaves;
    public float persistence; public float smoothing;

    public Noisy(int seed, float octaves, float persistence) { this.seed = seed;
        this.octaves = octaves; this.persistence = persistence; this.smoothing = 4.0f;
    }

    public Noisy(int seed, float octaves) { this(seed, octaves, .25f);
    }

    public Noisy(int seed) { this(seed, 4, .25f);
    }
    ...
    public float rawNoise2D(float x, float y) { return rawNoise(x + y * 57);
    }

    public float smoothNoise(float x) { float left = rawNoise(x - 1.0f); float right = rawNoise(x + 1.0f);
        return (rawNoise(x) / 2.0f) + (left / smoothing) + (right / smoothing);
    }

    public float smoothNoise2D(float x, float y) {
        float corners = rawNoise2D(x - 1, y - 1) + rawNoise2D(x - 1, y + 1) + rawNoise2D(x + 1, y - 1)
                + rawNoise2D(x + 1, y + 1);
    ...
    public float linearInterpolate(float a, float b, float x) { return a * (1 - x) + b * x;
    }

    public float cosineInterpolate(float a, float b, float x) { float f = (1.0f - (float) Math.cos(x * Math.PI)) / 2.0f; return a * (1 - f) + b * f;
    }

    public float interpolateNoise(float x) {
        return cosineInterpolate(smoothNoise((float) Math.floor(x)), smoothNoise((float) Math.floor(x) + 1.0f),
    ...
    public float interpolateNoise2D(float x, float y) {
        float a = cosineInterpolate(
                ...
                smoothNoise2D((float) Math.floor(x) + 1.0f, x - (float) Math.floor(x));

        float b = cosineInterpolate( smoothNoise2D(
                (float) Math.floor(x), (float) Math.floor(y) + 1),
