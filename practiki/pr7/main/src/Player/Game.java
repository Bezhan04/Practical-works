...
public class Game {
    ...
    public void distributeAsExample() {
        ...
        Collections.sort((List<Card>) Card.getCards(), new Comparator<Card>() {
            ...
            public int compare(Card c1, Card c2) {
                return c1.getValue() > c2.getValue() ? 1 : (c1.getValue()
                        < c2.getValue()) ? -1 : 0;
            }
        });

        for (int i = 1; i < Card.getCards().size(); i += 2) player1.pushCard(Card.getCards().get(i));

        for (int i = 2; i < Card.getCards().size(); i += 2) player2.pushCard(Card.getCards().get(i));
    ...
    private Player isEnd() {
        ...
        else if (!player2.hasCards()) return player1;
        else
            return null;
    }
