...
import student.exception.EmptyStringException;
import student.exception.StudentNotFoundException;
import student.sorting.Student;
...
public class StudentManager {
    ...
    public static ArrayList<Student> sort(ArrayList<Student> array, int left, int right,
        ...
        if (rr > ll) {
            ...
            while (ll <= rr) {
                while (ll < right && comparator.compare(array.get(ll), pivot) < 0) ll += 1;
                while (rr > left && comparator.compare(array.get(rr), pivot) > 0) rr -= 1;
                if (ll <= rr) {
